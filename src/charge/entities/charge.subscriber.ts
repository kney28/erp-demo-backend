...
import { ChargeLogs, LogChargeType } from './chargelogs.entity';

@EventSubscriber()
export class ChargeSubscriber implements EntitySubscriberInterface<Charge> {
  constructor(
  ...
  createLog(
    type: LogChargeType,
    event: InsertEvent<Charge> | UpdateEvent<Charge> | SoftRemoveEvent<Charge>,
  ) {
    ...
    const chargelogs: ChargeLogs = this.chargelogsRepository.create(chargelog);
